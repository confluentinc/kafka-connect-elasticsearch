...
public class Mapping {
   ...
   * @return the schema as a mapping structure, suitable for JSON serialization and use as the
   *     body of an Elasticsearch put-mapping request
   */
  ...
  private static Map<String, Object> buildMap(Schema schema) {
    Map<String, Object> keyMapping = new LinkedHashMap<>();
    buildMapping(schema.keySchema(), keyMapping);

    Map<String, Object> valueMapping = new LinkedHashMap<>();
    buildMapping(schema.valueSchema(), valueMapping);

    Map<String, Object> properties = new LinkedHashMap<>();
    properties.put(KEY_FIELD, keyMapping);
    properties.put(VALUE_FIELD, valueMapping);
  ...
  private static Map<String, Object> buildStruct(Schema schema) {
    ...
    for (Field field : schema.fields()) {
      ...
      buildMapping(field.schema(), fieldMapping);
  ...
  private static Map<String, Object> inferPrimitive(String type, Object defaultValue) {
    ...
    mapping.put(TYPE_FIELD, type);
    if (type.equals(TEXT_TYPE)) {
      mapping.put(FIELDS_FIELD, addTextMapping());
    }
    ...
    switch (type) {
      case BYTE_TYPE:
        mapping.put(DEFAULT_VALUE_FIELD, (byte) defaultValue);
        break;
      case SHORT_TYPE:
        mapping.put(DEFAULT_VALUE_FIELD, (short) defaultValue);
        break;
      case INTEGER_TYPE:
        mapping.put(DEFAULT_VALUE_FIELD, (int) defaultValue);
        break;
      case LONG_TYPE:
        mapping.put(DEFAULT_VALUE_FIELD, (long) defaultValue);
        break;
      case FLOAT_TYPE:
        mapping.put(DEFAULT_VALUE_FIELD, (float) defaultValue);
        break;
      case DOUBLE_TYPE:
        mapping.put(DEFAULT_VALUE_FIELD, (double) defaultValue);
        break;
      case BOOLEAN_TYPE:
        mapping.put(DEFAULT_VALUE_FIELD, (boolean) defaultValue);
        break;
      case DATE_TYPE:
        mapping.put(DEFAULT_VALUE_FIELD, ((java.util.Date) defaultValue).getTime());
        break;
      /*
